...
PAGE_METADATA: dict[str, dict[str, Any]] = {
    ...
    "guide/extraction-duplicates/index.html": {
        ...
        "result": ["결과 목록이 비어 있으면 설정한 최소 횟수 이상 반복된 원문이 없다는 뜻입니다.", "중복 횟수와 파일별 등장 횟수를 보고 실제로 같은 번역을 써도 되는 문구인지 확인합니다.", "수정 또는 번역 후에는 대상 추출 결과에서 해당 문구가 의도대로 바뀌었는지 확인합니다.", "번역까지 진행했다면 이후 일반 번역/적용 흐름에서 결과가 어긋나지 않는지 작은 범위로 확인합니다.", "수동 저장 후 결과가 마음에 들지 않으면 가능한 경우 되돌리기를 사용합니다."],
