...
# Git / GitHub Rules
- 브랜치명을 만들 때 `codex/` 같은 도구명 접두사는 사용하지 않는다. 사용자가 명시하지 않았으면 저장소 목적에 맞는 짧은 이름을 쓰거나 먼저 확인한다.
- 외부 스킬이나 도구의 기본 브랜치명 관례보다 사용자의 저장소 네이밍 취향을 우선한다.
- 커밋/푸시 전에는 공개 커밋에 포함할 파일과 로컬 컨텍스트 파일을 분리해서 확인한다.
