(function(){
  window.__AIMT_GUIDE_MAIN_VERSION = "search-url-cache-fix-2026-06-22-02";
  console.log("[AIMT guide main loaded]", window.__AIMT_GUIDE_MAIN_VERSION, location.href);
  ...
  function clampSidebarWidth(value){
    const maxByViewport = Math.max(
      minSidebarWidth,
      Math.min(maxSidebarWidth, Math.floor(window.innerWidth * 0.48))
    );
  ...
  function readNavState(){
    try {
      return JSON.parse(localStorage.getItem(navStateKey) || "{}");
    } catch (_) {
      return {};
    }
  }
  ...
  function navGroupKey(group){
    ...
    const legacyKey = link
      ? new URL(link.getAttribute("href"), location.href).pathname.replace(/\/index\.html$/, "/")
      : "";
  ...
  function writeSidebarScroll(value){
    try {
      sessionStorage.setItem(sidebarScrollKey, String(Math.max(0, Math.round(value))));
    } catch (_) {}
  }
  ...
  function setupNavGroups(){
    ...
    document.querySelectorAll(".nav-group").forEach((group) => {
      ...
      } else if (legacyKey && Object.prototype.hasOwnProperty.call(state, legacyKey)) {
        group.open = Boolean(state[legacyKey]);
      }
    ...
    requestAnimationFrame(() => {
      ...
      if (savedScroll === null || linkRect.top < sidebarRect.top || linkRect.bottom > sidebarRect.bottom) {
        sidebar.scrollTop = Math.max(
          0,
          currentLink.offsetTop - Math.round((sidebar.clientHeight - currentLink.offsetHeight) / 2)
        );
      }
    });
  ...
  function isGuideDocumentUrl(url){
    ...
    return (
      ...
      (path.endsWith("/") || path.endsWith(".html") || path.endsWith(".htm"))
    );
  }

  function normalizeGuideUrl(urlValue){
    const url = new URL(urlValue, location.href);
    let path = url.pathname.replace(/\\/g, "/");

    /*
      /guide/가 여러 번 등장하는 모든 경우를 정리한다.
    ...
    */
    const marker = "/guide/";
    const firstGuide = path.indexOf(marker);
    const lastGuide = path.lastIndexOf(marker);

    if (firstGuide >= 0 && lastGuide > firstGuide) {
      path = path.slice(0, firstGuide + marker.length) + path.slice(lastGuide + marker.length);
    }

    /*
      혹시 바로 이어진 중복도 한 번 더 정리.
    */
    path = path.replace(/\/guide\/(?:guide\/)+/gi, "/guide/");
  ...
  async function replaceArticleFromUrl(url, options){
    url = new URL(normalizeGuideUrl(url.href || url));
  ...
  function setupPartialNavigation(){
    ...
    document.addEventListener("click", (event) => {
      ...
      try {
        url = new URL(normalizeGuideUrl(link.getAttribute("href")));
      } catch (_) {
        return;
      }
      ...
      replaceArticleFromUrl(url, {push: true}).catch(() => {
        location.href = url.href;
      });
    }, true);

    window.addEventListener("popstate", () => {
      replaceArticleFromUrl(new URL(normalizeGuideUrl(location.href)), {push: false}).catch(() => {
        location.reload();
      });
    });
  }
  ...
  async function setupSearch(){
    ...
    const scriptSrcAttribute = script.getAttribute("src");
    const scriptSrc = scriptSrcAttribute
      ? new URL(scriptSrcAttribute, initialBaseUri).href
      : (script.src || initialBaseUri);

    function resolveGuideRootUrl(scriptSrc){
      const currentUrl = new URL(location.href);
      const currentPath = currentUrl.pathname.replace(/\\/g, "/");
      const guideMarker = "/guide/";
      const guideIndex = currentPath.indexOf(guideMarker);

      if (guideIndex >= 0) {
        currentUrl.pathname = currentPath.slice(0, guideIndex + guideMarker.length);
        currentUrl.search = "";
        currentUrl.hash = "";
      ...
      const markers = ["/assets/", "/static/"];

      for (const marker of markers) {
        const markerIndex = scriptPath.indexOf(marker);

        if (markerIndex >= 0) {
          scriptUrl.pathname = scriptPath.slice(0, markerIndex + 1);
          scriptUrl.search = "";
          scriptUrl.hash = "";
    ...
    const guideRootUrl = resolveGuideRootUrl(scriptSrc);
    const searchIndexUrl = new URL("search-index.json", guideRootUrl).href;
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("검색 인덱스를 불러올 수 없습니다: " + response.status + " " + searchIndexUrl);
      }

      index = await response.json();
    } catch (error) {
      console.warn("[setupSearch] search-index load failed", error);
      return;
    }

    function cleanSearchPath(value){
      ...
      while (path.startsWith("dist/")) {
        path = path.slice("dist/".length);
      }

      while (path.startsWith("guide/")) {
        path = path.slice("guide/".length);
      }
    ...
    function searchResultUrl(item){
      const urlPath = cleanSearchPath(item.url);
      const filePath = cleanSearchPath(item.path);
      const pagePath = urlPath || filePath;
      const url = new URL(pagePath || "index.html", guideRootUrl);

      url.search = "";
      url.hash = "";
      return normalizeGuideUrl(url.href);
    }
    ...
    function scoreItem(item, query, compactQuery, tokens){
      ...
      const titleMatched = Boolean(query) && (
        title.includes(query) ||
        tightTitle.includes(compactQuery) ||
        tokens.some((token) => title.includes(token) || tightTitle.includes(token))
      );

      const bodyMatched = Boolean(query) && (
        body.includes(query) ||
        tightBody.includes(compactQuery) ||
        tokens.some((token) => body.includes(token) || tightBody.includes(token))
      );
    ...
    function renderSearchResults(){
      ...
      const matches = index
        .map((item) => scoreItem(item, query, compactQuery, tokens))
      ...
      for (const row of matches) {
        ...
        if (row.titleMatched) {
          badges.appendChild(makeBadge("제목"));
        }

        if (row.bodyMatched) {
          badges.appendChild(makeBadge("내용"));
        }
